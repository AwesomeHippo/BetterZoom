...
public class ZoomHandler {
    ...
    public static void onFOVChange(ViewportEvent.ComputeFov event) {
        ...
        float targetZoomFactor = isZooming ? Math.clamp(zoomFOV / playerFov, 0.01f, 1.0f) : 1.0f;

        // smooth the zoom or not
        if (Config.SMOOTH_ZOOM.get()) {
            float easing = Config.SMOOTH_EASING_FACTOR.get().floatValue();
            float dt = Math.max(mc.getTimer().getGameTimeDeltaTicks(), 0f);
            ...
            if (Config.AUTO_ADJUST_SENSITIVITY.get()) {
                ...
                if (isZooming) {
                    ...
                    if (start >= 1.0f - 1e-6f) {
                        ...
                        progress = Math.clamp((smoothedZoomFactor - start) / (1.0f - start), 0f, 1.0f);
                    }
